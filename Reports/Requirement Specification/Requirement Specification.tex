...
This project is for Professor Alexandra Cristea who shall henceforth be referred to as 'the client'. The client has given us the project of developing a VR meditation application with the possible use as the software basis for research into the topic.
...
\begin{itemize}
	...
	The client would prefer for the project to personalise, that is automatic adjustments\footnote{Due to the early stage of the project, adjustments here is used in a broad sense in relation to general environmental changes such as colours. This choice was partly inspired by the research aspect of the project.} to suit the user, itself rather than have the user customise the project, that is allowing the user to alter the environment
	\item Stability\\
	The client would prefer fewer stable features over more less-stable features
	\item Modularity\\
	The client would prefer the software to be modular\footnote{Modular here refers to the practice of ensuring code is written in such a way that sections can be reused in different projects with minimal overhead or time required for implementation} to allow for ease of reuse in future projects
\end{itemize}
...
The server itself will be run by a Python script that can be run on any computer. For the purpose of product demonstration, we will ensure the same computer runs both the server and Quest app in lieu of a server with a static IP. The server will store user account data, raw session data, and session metric results. Data will be stored in a XML format, with each user having a folder under their username, with sessions and user data within that folder. XML has been chosen for data storage due to it's wide use and simplicity. XML also allows for data type definitions for which current incomplete DTD are given in \cref{apdx:xml_dtd}. this wide and simple use ensures a large number of available tools to work with XML and is scalable beyond the expected required number of users thus causing little to no scalability issues.
...
For completeness, we note that this is a novel project with the client not currently using any similar system for purposes similar to the purposes of our application.
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}[
		...
		\node[node](nfr-2)[below of=nfr-1,xshift=-17mm]{NFR-2};
		\node[node](nfr-3)[below of=nfr-1,xshift=17mm]{NFR-3};
